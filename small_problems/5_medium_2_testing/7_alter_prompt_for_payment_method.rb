...
class CashRegisterTest < Minitest::Test
  ...
  def test_prompt_for_payment
    amount = StringIO.new('120')
    assert_output do
      @valid_transaction.prompt_for_payment(input: amount)
    end
    assert_equal(120, @valid_transaction.amount_paid)
  end
  ...
  def test_prompt_for_payment
    ...
    output = StringIO.new
    assert_output do
      @valid_transaction.prompt_for_payment(input: input, output: output)
    end
    assert_equal(120, @valid_transaction.amount_paid)
  end
